...
export default function AdminDashboard() {
  ...
  return (
    <div className="animate-in">
      <div className="mb-7">
        <p className="text-xs uppercase tracking-[0.22em] font-bold mb-2" style={{ color: 'var(--color-teal)' }}>{t('admin.control')}</p>
        <h1 className="font-display text-2xl sm:text-3xl font-extrabold" style={{ color: 'var(--color-ink)' }}>{t('nav.dashboard')}</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>{t('admin.subtitle')}</p>
      </div>

      <PWAInstallCard roleLabel={t('role.admin')} />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard label={t('nav.branches')} value={stats?.jami_filiallar ?? 0} icon={Building2} accent="teal" delay={0} />
          <StatCard label={t('admin.supervisors')} value={stats?.jami_nazoratchilar ?? 0} icon={Users} accent="amethyst" delay={80} />
          <StatCard label={t('admin.students')} value={stats?.jami_oquvchilar ?? 0} icon={GraduationCap} accent="jungle" delay={160} />
          <StatCard label={t('nav.certificates')} value={certificates.length} icon={Award} accent="olive" delay={240} />
        </div>
      )}
      ...
      {!loading && certificates.length > 0 && (
        <Card className="p-5 mb-6 certificate-notification animate-in-fast">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              <div className="flex items-center gap-2"><Award size={18} style={{ color: 'var(--color-olive)' }} /><h2 className="font-display font-bold" style={{ color: 'var(--color-ink)' }}>{t('admin.newGraduates')}</h2></div>
              <p className="text-xs mt-1" style={{ color: 'var(--color-muted)' }}>{t('admin.newGraduatesText')}</p>
            </div>
            <Link to="/admin/sertifikatlar"><Button size="sm" variant="secondary">{t('admin.viewAll')} <ArrowRight size={14} /></Button></Link>
          </div>
      ...
      <Card className="animate-in-fast">
        <div className="flex items-center gap-2 p-5 pb-0"><TrendingUp size={17} style={{ color: 'var(--color-jungle)' }} /><h2 className="font-display font-bold text-sm" style={{ color: 'var(--color-ink)' }}>{t('admin.branchStats')}</h2></div>
        {loading ? (
          <div className="p-5 space-y-2">{[1, 2, 3].map((i) => <Skeleton key={i} className="h-10 w-full" />)}</div>
        ) : !stats?.filiallar_kesimida?.length ? (
          <p className="text-sm py-8 text-center px-5" style={{ color: 'var(--color-muted)' }}>{t('admin.noBranches')}</p>
        ) : (
          <div className="overflow-x-auto p-5 pt-3">
            <table className="w-full text-sm">
              <thead><tr className="text-left border-b" style={{ borderColor: 'var(--color-line)' }}><th className="py-2 pr-4">{t('admin.branch')}</th><th className="py-2 pr-4">{t('admin.supervisors')}</th><th className="py-2 pr-4">{t('admin.students')}</th><th className="py-2 pr-4">{t('admin.averageResult')}</th></tr></thead>
