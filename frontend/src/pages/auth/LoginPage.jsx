...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // xato AuthContext ichida ko'rsatiladi
    }
  };
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--color-paper)' }}>
      ...
      >
        ...
        />
        <div
          className="absolute -top-24 -right-24 w-96 h-96 rounded-full opacity-[0.08] animate-float"
          style={{ background: 'var(--color-amber)' }}
        />
        <div
          className="absolute bottom-0 left-0 w-72 h-72 rounded-full opacity-[0.06] animate-float"
          style={{ background: 'var(--color-moss)', animationDelay: '1s' }}
        />

        <div className="relative z-10 flex items-center gap-3 animate-in-fast">
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, var(--color-amber) 0%, var(--color-amber-dark) 100%)', boxShadow: '0 4px 16px rgba(199,130,42,0.35)' }}
          >
            <GraduationCap size={22} color="white" strokeWidth={2.2} />
          </div>
          <span className="font-display text-white text-lg font-bold tracking-tight">Bilim Yo'li</span>
        </div>

        <div className="relative z-10">
          <h1 className="font-display text-4xl font-bold text-white leading-tight mb-4 animate-in-fast stagger-1">
            Har bir mavzu,<br />har bir daraja,<br />o'z o'rnida.
          </h1>
          <p className="text-white/70 text-base leading-relaxed max-w-sm animate-in-fast stagger-2">
            Filiallar, nazoratchilar va o'quvchilar — bitta tizimda, aniq nazorat ostida.
          </p>
        </div>
        ...
        <div className="relative z-10 flex items-center gap-6 animate-in-fast stagger-3">
          {[
            ...
            { icon: Users, label: 'Nazoratchilar' },
        ...
        <div className="w-full max-w-sm animate-in">
          <div className="lg:hidden flex items-center gap-2 mb-8 justify-center animate-pop">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, var(--color-forest) 0%, var(--color-forest-dark) 100%)' }}
            >
              <GraduationCap size={18} color="white" />
            </div>
            <span className="font-display text-lg font-bold" style={{ color: 'var(--color-forest)' }}>Bilim Yo'li</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="animate-in-fast stagger-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 press"
                  style={{ color: '#9C9584' }}
                  tabIndex={-1}
                >
            ...
            {error && (
              <div
                className="text-sm px-3 py-2.5 rounded-xl animate-pop"
                style={{ background: '#FBEAE8', color: 'var(--color-red)' }}
              >
                {error}
              </div>
            )}
