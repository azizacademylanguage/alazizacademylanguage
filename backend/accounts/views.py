...
class ParolAlmashtirishView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        eski_parol = request.data.get('eski_parol', '')
        yangi_parol = request.data.get('yangi_parol', '')
        yangi_parol2 = request.data.get('yangi_parol2', '')
        ...
        if len(yangi_parol) < 8:
            return Response({'detail': "Yangi parol kamida 8 ta belgidan iborat bo'lsin."}, status=status.HTTP_400_BAD_REQUEST)
        if yangi_parol != yangi_parol2:
            return Response({'detail': "Yangi parollar bir xil emas."}, status=status.HTTP_400_BAD_REQUEST)
        request.user.set_password(yangi_parol)
        request.user.token_version += 1
        request.user.save(update_fields=['password', 'token_version'])
        return Response({'detail': "Parol almashtirildi. Barcha qurilmalarda qayta kirish kerak."})


class BarchaQurilmalardanChiqishView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        request.user.token_version += 1
        request.user.save(update_fields=['token_version'])
        return Response({'detail': 'Barcha qurilmalardagi sessiyalar bekor qilindi.'})


class MeningXavfsizligimView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        from .models import KirishTarixi
        tarix = KirishTarixi.objects.filter(username=request.user.username).order_by('-created_at')[:20]
        return Response({
            'token_version': request.user.token_version,
            'oxirgi_kirish': request.user.last_login,
            'kirishlar': [
                {
                    'id': item.id,
                    'muvaffaqiyatli': item.muvaffaqiyatli,
                    'ip_manzil': item.ip_manzil,
                    'qurilma': item.user_agent,
...
class AdminXavfsizlikView(APIView):
    permission_classes = [IsAdmin]
    ...
    def get(self, request):
        ...
        tarix = KirishTarixi.objects.select_related('user').order_by('-created_at')[:100]
        return Response({
            'muvaffaqiyatli_24_soat': KirishTarixi.objects.filter(muvaffaqiyatli=True, created_at__gte=since).count(),
            'xato_24_soat': KirishTarixi.objects.filter(muvaffaqiyatli=False, created_at__gte=since).count(),
            'faol_foydalanuvchilar': User.objects.filter(faol=True).count(),
            'nofaol_foydalanuvchilar': User.objects.filter(faol=False).count(),
            'kirishlar': [
                {
                    'id': item.id,
                    ...
                    'role': item.user.role if item.user else '',
                    'muvaffaqiyatli': item.muvaffaqiyatli,
                    'ip_manzil': item.ip_manzil,
                    'qurilma': item.user_agent,
                    'created_at': item.created_at,
...
class AdminSessiyalarniBekorQilishView(APIView):
    permission_classes = [IsAdmin]
    ...
    def post(self, request, user_id):
        ...
        return Response({'detail': f'{user.full_name} barcha qurilmalardan chiqarildi.'})
